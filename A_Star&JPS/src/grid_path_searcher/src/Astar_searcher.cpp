...
{   
    ...
    */
    ...
    Eigen::Vector3i theNode=currentPtr->index;
    int the_x=theNode[0];
    int the_y=theNode[1];
    int the_z=theNode[2];
    auto the_coord=currentPtr->coord;
    int  n_x, n_y, n_z;
    double dist;
    ...
    for(int i=-1;i<=1;++i)
    {
        for(int j=-1;j<=1;++j)
        {
            for(int k=-1;k<=1;++k)
            {
                ...
                if((std::abs(n_coord[0]-the_coord[0])<1e-6)&&(std::abs(n_coord[1]-the_coord[1])<1e-6)&&(std::abs(n_coord[2]-the_coord[2])<1e-6))
                {
                    std::cout << "Error: Not expanding correctly" << std::endl;
                    std::cout << "n_coord:" << n_coord[0] << " "<<n_coord[1]<<" "<<n_coord[2] << std::endl;
                    std::cout << "this_coord:" << the_coord[0] << " "<<the_coord[1]<<" "<<the_coord[2] << std::endl;

                    std::cout << "current node index:" << the_x << " "<< the_y<<" "<< the_z << std::endl;
                    std::cout << "neighbor node index:" << n_x << " "<< n_y<<" "<< n_z << std::endl;
                }

                dist=std::sqrt((n_coord[0] - the_coord[0]) * (n_coord[0] - the_coord[0])+
                               (n_coord[1] - the_coord[1]) * (n_coord[1] - the_coord[1])+
                               (n_coord[2] - the_coord[2]) * (n_coord[2] - the_coord[2]));
    ...
    */
    ...
    while ( !openSet.empty() )
    {
        ...
        */
        ...
        //multimap会根据key值自动排序，这里移除最小f值对应的node
        openSet.erase(openSet.begin());
        //将但前最小f值对应的node标记为已访问，放入close set
