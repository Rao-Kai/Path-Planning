...
double Homeworktool::OptimalBVP(Eigen::Vector3d _start_position,Eigen::Vector3d _start_velocity,Eigen::Vector3d _target_position)
{
    ...
    */
    ...
    // 求解一元四次方程 T^4 - 4*(vx0^2 + vy0^2 + vz0^2) * T^2 + 24 * (dx * vx0 + dy * vy0 + dz * vz0) * T - 36 * (dx^2 + dy^2 +dz^2)
    // 通过求解伴随矩阵的特征值来求解方程，加快计算速度
    // 想法来源: https://blog.csdn.net/fb_941219/article/details/102984587
    
    Eigen::Matrix<double, 4, 4> matrix_a;  
    ...
    std::cout<<"matrix_a: "<<std::endl<<matrix_a<<std::endl<<std::endl;

    matrix_eigenvalues = matrix_a.eigenvalues();
    ...
    optimal_cost  = *min_element(j.begin(), j.end()); // 获取最优的J
